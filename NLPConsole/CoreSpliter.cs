...
namespace NLP
{
    public class CoreSpliter
    {
        public static CoreSegment Split(string content)
        {
            ...
            // 设置参数
            segment.Subsegs[0].Index = 0;
            segment.Subsegs[0].Flag = 0x100;
        ...
        private static void MinSplit(CoreSegment segment)
        {
#if DEBUG
            Debug.Assert(segment != null && segment.Length > 0);
            ...
            // 排序（按照计数排序）
            segments = segments.OrderBy(s => s.Count).ToList();
            // 检查结果
            // 没有查询到结果
            if (segments.Count <= 0)
            {
            ...
            {
                if(item != middleSegment) MinSplit(item);
            }
        }
    }
}
